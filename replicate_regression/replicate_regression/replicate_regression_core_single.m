...
%[result, parameters, options, sample] = replicate_regression_core_single(t,y,sigma,tt,options)
%
...
switch options.basis
  case {'cos', 'sin', 'sin half', 'sin horizontal'},
...
switch options.basis
  ...
  case {'cos','sin','sin_half','sin_horizontal','polynomial','exp','cos+sin'},
...
theta_prior_std         = sigma_alpha;
x_mean                  = y';
x_std                   = sigma;

if options.use_L_one_norm ==0,
  %% to avoid problems with ill-conditioned posterior covariance matrix:
  epsilon                 = 10^-5; 
  theta_prior_cov_inv     = diag(1./theta_prior_std.^2);
  x_cov_inv               = diag(1./x_std.^2);
  ...
  if options.verbose, display('Using L1 norm'); end
  [theta_posterior_mean, ~, ~, ~, theta_posterior_cov] = maximise_bayesian_posterior_L1(x_mean, x_std, theta_prior_mean, theta_prior_std, M);
end
...
result.x_average     = x;
result.sigma         = sigma;
result.sigma_central = sigma;
result.sigma_average = sigma;
result.x_fit         = x_fit;
result.t             = tt;
