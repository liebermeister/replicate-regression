...
if options.show_central,
  if length(result),
    if ~options.show_bands,
      h = plot_range(t_reg,x_reg,[x_reg-sigma_reg; x_reg+sigma_reg],[],[0 0 0],[0.9 0.9 0.9],[],[],':'); hold on;
    end
...
if options.show_bands,
  plot_range(t_reg,x_reg_replicate{1},[x_reg_replicate{1}-sigma_reg_replicate{1}; x_reg_replicate{1}+sigma_reg_replicate{1}],[],colors{1},[],[],0.7,'',2); hold on;
  plot_range(t_reg,x_reg_replicate{2},[x_reg_replicate{2}-sigma_reg_replicate{2}; x_reg_replicate{2}+sigma_reg_replicate{2}],[],colors{2},[],[],0.7,'',2); hold on;
  plot_range(t_reg,x_reg_replicate{3},[x_reg_replicate{3}-sigma_reg_replicate{3}; x_reg_replicate{3}+sigma_reg_replicate{3}],[],colors{3},[],[],0.7,'',2);hold on;
else,
...
if options.show_errorbars,
  if length(t1), h = [h errorbar(t1,y1,sigma1,'.','MarkerSize',30,'Color',colors{1})];  legends =[legends {'Data 1'}]; end
  if length(t2), h = [h errorbar(t2,y2,sigma2,'.','MarkerSize',30,'Color',colors{2})];  legends =[legends {'Data 2'}]; end
  if length(t3), h = [h errorbar(t3,y3,sigma3,'.','MarkerSize',30,'Color',colors{3})];  legends =[legends {'Data 3'}]; end
else,
  if length(t1), h = [h plot(t1,y1,'.','MarkerSize',30,'Color',colors{1})];  legends =[legends {'Data 1'}]; end
  if length(t2), h = [h plot(t2,y2,'.','MarkerSize',30,'Color',colors{2})];  legends =[legends {'Data 2'}]; end
  if length(t3), h = [h plot(t3,y3,'.','MarkerSize',30,'Color',colors{3})];  legends =[legends {'Data 3'}]; end
end
