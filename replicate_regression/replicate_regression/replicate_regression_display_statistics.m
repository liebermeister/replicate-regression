...
% show at most 1000 points
nnn = min(1000,prod(size(result.statistics.residuals)));
vv    = result.statistics.residuals(1:nnn);
sigma = result.statistics.sigma(1:nnn);
figure(25); clf; set(gca, 'Fontsize',24);
plot(sigma,vv,'.','Color',[0.05, 0, 0.8]); hold on
plot([10 0 10],[10 0 -10],'k--'); axis equal; axis([-0.1,1.5,-1.5,1.5]);
