...
switch options.basis,
  case 'cos+sin',
    names = [{'Offset'}; repmat({'Mode'},2*length(statistics.alpha.std),1);{'Jump'}];    
...
if options.update_prior_means,
  ...
  switch options.basis,
    case 'cos+sin',
      options_update.central_mode_mean  = repmat(options_update.central_mode_mean,1,2); 
      options_update.deviation_mode_mean  = repmat(options_update.deviation_mode_mean,1,2); 
    otherwise
  end
