function [data_reg, options_complete, options_update, crossvalidation_error] = replicate_regression_complete(data_replicates,options,foptions)
...
% INPUT ARGUMENT 
% 'data_replicates' 
%    contains the replicate data sets as fields (all in biotable format, same form!!)
%    The entries of the first field are used as unique identifiers
%    The data entries are reordered in the combined data set.
%  'options'                        
%    options for multi-curve regression (see script replicate_regression)
%    with optional additional fields .prior_updating and .updating_factor
%
% OUTPUT VARIABLES
%  'data_reg' with fields (struct arrays):
%    IN ORIGINAL TIME SCALE: 
%      data_reg.combined  : data structure from intermediate processing steps
%                         field 'SampleName' contains replicate labels (as numbers)
%      data_reg.fit         fitted data points (reordered!)
%     crossvalidation_average, crossvalidation_replicate, pooled_crossvalidation, naive_crossvalidation
%
%    IN NEW TIME SCALE
%      data_reg.average, data_reg.central, data_reg.replicates: regression results
%
%    data_reg.statistics:                     additional information about the regression procedure
...
if run_updating,
  ...
  for it = 1:options.prior_updating,
    ...
    % last round:
    if it == options.prior_updating;
      if length(foptions.updating_factor_final),
        my_options.updating_factor = foptions.updating_factor_final;
      end
    end
...
%if foptions.run_crossvalidation,
  crossvalidation_error.mcr_replicate = nanmean([data_reg.combined.DataMean(:) - data_reg.crossvalidation_replicate.DataMean(:)].^2);
  crossvalidation_error.mcr_average   = nanmean([data_reg.combined.DataMean(:) - data_reg.crossvalidation_average.DataMean(:)].^2);
  crossvalidation_error.pooled        = nanmean([data_reg.combined.DataMean(:) - data_reg.pooled_crossvalidation.DataMean(:)].^2);
  crossvalidation_error.naive         = nanmean([data_reg.combined.DataMean(:) - data_reg.naive_crossvalidation.DataMean(:)].^2);
else
  crossvalidation_error.mcr_replicate = [];
  crossvalidation_error.mcr_average=[];
  crossvalidation_error.pooled = [];
  crossvalidation_error.naive = [];
end
...
if length(foptions.log_file),
  ...
  fprintf(file,'Crossvalidation replicate: %f\n', crossvalidation_error.mcr_replicate);
  fprintf(file,'Crossvalidation average: %f\n', crossvalidation_error.mcr_average);
  fprintf(file,'Crossvalidation pooled: %f\n', crossvalidation_error.pooled);
  fprintf(file,'Crossvalidation naive regression: %f\n', crossvalidation_error.naive);
  fclose(file);
end
...
if foptions.run_crossvalidation, 
  deviation  = abs(data_reg.crossvalidation_replicate.DataMean - data_reg.combined.DataMean);
  % data on log scale:      count abs deviation between data and prediction (from crossvalidation)
  % data on absolute scale: normalise the deviation by mean(data point, median of protein)
  switch  foptions.data_scale,
    case {'abs','absolute'},
      deviation  = deviation ./ repmat(nanmedian(data_reg.combined.DataMean')',1,size(data_reg.combined.DataMean,2));
  end
