  ...
  if sum(isfinite(x_mean)),
    ...
    if ~p.flag_only_data,
      if p.flag_omit_replicates,
        ...
        plot(column(averaged.SampleTime),column(x_upper),':','Color', [0 0 0],'Linewidth',1); hold on;
        plot(column(averaged.SampleTime),column(x_lower),':','Color', [0 0 0],'Linewidth',1); hold on
      end
    end
