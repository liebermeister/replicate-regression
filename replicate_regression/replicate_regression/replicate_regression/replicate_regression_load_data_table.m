...
foptions_default = struct('data_std_log', 0.3, 'data_std_relative', 0.3, 'translation_table_file',[], 'convenience_name', [], 'data_scale', [], 'data_outliers_upper',inf,'data_outliers_lower',-inf,'ignore_std_deviations',1);
...
% increase std dev for outlier values by a factor of 3

if strcmp(foptions.data_scale,'absolute')
  StdDev(Values>foptions.data_outliers_upper) = 3 * StdDev(Values>foptions.data_outliers_upper);
  StdDev(Values<foptions.data_outliers_lower) = 3 * StdDev(Values<foptions.data_outliers_lower);
end
