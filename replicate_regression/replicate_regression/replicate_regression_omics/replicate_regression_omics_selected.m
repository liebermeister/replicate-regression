...
% foptions_file = [replicate_regression_PATH_NAME '/options_hahne_salt_stress_tagD.csv'];


% ----------------------------------------------------------------------------------------

foptions_default = struct('plot_data', 1,'plot_regression',1,'plot_replicates', 1,'plot_all',0,...
                          'graphics_format', 'png', ...
                          'graphics_scale','absolute','title_string',[],'delimiter_symbol','|',...
                          'x_label','Time','y_label','','convenience_name',[],'postprocess_normalise',0,...
                          'log_transformation','arithmetic','name_print_capital',0);
...
if isfield(foptions,'element_id'),
  ...
  %% next two lines are just for safety (possibly missing names)
...
if foptions.plot_all,
  ...
  if isfield(foptions,'mark_outliers_percentage'),
